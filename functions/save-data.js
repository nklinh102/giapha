...
// ... (Hàm isValidToken không cần sửa) ...
...
  const { request, env } = context;
  const headers = { 'Content-Type': 'application/json' }; // <-- Thêm dòng này

  if (!(await isValidToken(request, env))) {
    // SỬA DÒNG NÀY
    return new Response(JSON.stringify({ message: "Xác thực thất bại." }), { status: 401, headers: headers });
  }
...
  } catch (e) {
    // SỬA DÒNG NÀY
    return new Response(JSON.stringify({ message: e.message }), { status: 400, headers: headers });
  }
...
    });
    ...
    // SỬA DÒNG NÀY
    return new Response(JSON.stringify({ message: `Đã lưu ${filePath} thành công!` }), { status: 200, headers: headers });
  } catch (error) {
    console.error("Save error:", error);
    // SỬA DÒNG NÀY
    return new Response(JSON.stringify({ message: "Lỗi khi lưu: " + error.message }), { status: 500, headers: headers });
  }
}
